...
const DEFAULT_QQ_FRAMEWORK_SYNC_CONFIG = {
    serverUrl: '',
    apiToken: '',
    autoReconnect: false
};
...
function loadGlobalConfig() {
    ...
    try {
        ...
        // QQ 框架自动同步配置
        if (data.qqFrameworkSyncConfig && typeof data.qqFrameworkSyncConfig === 'object') {
            globalConfig.qqFrameworkSyncConfig = {
                serverUrl: String(data.qqFrameworkSyncConfig.serverUrl || DEFAULT_QQ_FRAMEWORK_SYNC_CONFIG.serverUrl).trim(),
                apiToken: String(data.qqFrameworkSyncConfig.apiToken || DEFAULT_QQ_FRAMEWORK_SYNC_CONFIG.apiToken).trim(),
                autoReconnect: data.qqFrameworkSyncConfig.autoReconnect === true
            };
        }
...
function getQqFrameworkSyncConfig() {
    return globalConfig.qqFrameworkSyncConfig
        ? { ...DEFAULT_QQ_FRAMEWORK_SYNC_CONFIG, ...globalConfig.qqFrameworkSyncConfig }
        : { ...DEFAULT_QQ_FRAMEWORK_SYNC_CONFIG };
}

function setQqFrameworkSyncConfig(config) {
    if (!config || typeof config !== 'object') return null;
    const current = getQqFrameworkSyncConfig();
    globalConfig.qqFrameworkSyncConfig = {
        serverUrl: String(config.serverUrl !== undefined ? (config.serverUrl || '') : current.serverUrl).trim(),
        apiToken: config.apiToken === undefined || config.apiToken === null || config.apiToken === ''
            ? current.apiToken
            : String(config.apiToken).trim(),
...
module.exports = {
    ...
    setCaptureConfig,
    DEFAULT_CAPTURE_CONFIG,
    getQqFrameworkSyncConfig,
    setQqFrameworkSyncConfig,
    DEFAULT_QQ_FRAMEWORK_SYNC_CONFIG,
    getDeviceProtocol,
    setDeviceProtocol,
    DEFAULT_DEVICE_PROTOCOL,
