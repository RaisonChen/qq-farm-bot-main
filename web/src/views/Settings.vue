...
const anySystemSaving = computed(() => systemSettingsSaving.value || systemConfigSaving.value || captureConfigSaving.value || qqFrameworkSyncConfigSaving.value || deviceProtocolSaving.value)
...
async function saveSystemSettings() {
  ...
  try {
    ...
    const [systemResult, captureResult, deviceResult, qfResult] = await Promise.all([
      ...
      api.post('/api/admin/qq-framework-sync-config', { ...localQqFrameworkSyncConfig.value, confirmed: true }),
    ])
    if (!systemResult.data?.ok || !captureResult.data?.ok || !deviceResult.data?.ok || !qfResult.data?.ok)
      throw new Error('部分系统配置保存失败')
    await Promise.all([loadSystemConfig(), loadCaptureConfig(), fetchDeviceProtocol(), loadQqFrameworkSyncConfig()])
...
onMounted(async () => {
  await Promise.all([loadSystemConfig(), loadCaptureConfig(), loadQqFrameworkSyncConfig()])
  await fetchAccounts()
      ...
      <div class="p-4">
        ...
        <div v-else-if="activeTab === 'system'" class="space-y-5">
          ...
          <AdminSystemPanel
            v-model:local-system-config="localSystemConfig"
            v-model:local-capture-config="localCaptureConfig"
            v-model:local-qq-framework-sync-config="localQqFrameworkSyncConfig"
            section="qq-framework-sync"
            :show-heading="false"
            :show-save="true"
            :default-system-config="defaultSystemConfig"
            :platform-options="platformOptions"
            ...
            :capture-config-saving="captureConfigSaving"
            :capture-config-testing="captureConfigTesting"
            :qq-framework-sync-config-saving="qqFrameworkSyncConfigSaving"
            :qq-framework-sync-syncing="qqFrameworkSyncSyncing"
            @save-qq-framework-sync="handleSaveQqFrameworkSyncConfig"
            @sync-all-qq-framework="handleSyncAllQqFramework"
          />
        </div>
      </div>
    </div>
