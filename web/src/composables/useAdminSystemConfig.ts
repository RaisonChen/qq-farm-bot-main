...
export interface QqFrameworkSyncConfig {
  serverUrl: string
  apiToken: string
  tokenConfigured: boolean
...
const defaultQqFrameworkSyncConfig: QqFrameworkSyncConfig = {
  serverUrl: '',
  apiToken: '',
  tokenConfigured: false,
  autoReconnect: false,
...
export function useAdminSystemConfig(options: UseAdminSystemConfigOptions) {
  ...
  const qqFrameworkSyncConfigSaving = ref(false)
  const qqFrameworkSyncSyncing = ref(false)
  ...
  const localQqFrameworkSyncConfig = ref<QqFrameworkSyncConfig>({ ...defaultQqFrameworkSyncConfig })
  ...
  async function loadQqFrameworkSyncConfig() {
    try {
      const { data } = await api.get('/api/admin/qq-framework-sync-config')
      if (data?.ok && data.data)
        localQqFrameworkSyncConfig.value = { ...defaultQqFrameworkSyncConfig, ...data.data, apiToken: '' }
    }
    catch (e: any) {
      console.error('加载 QQ 框架同步配置失败:', e)
    }
  }

  async function handleSaveQqFrameworkSyncConfig() {
    qqFrameworkSyncConfigSaving.value = true
    try {
      const { data } = await api.post('/api/admin/qq-framework-sync-config', {
        ...localQqFrameworkSyncConfig.value,
        confirmed: true,
      })
      if (data?.ok && data.data) {
        localQqFrameworkSyncConfig.value = { ...defaultQqFrameworkSyncConfig, ...data.data, apiToken: '' }
        options.showAlert('QQ 框架自动同步配置已保存', 'primary')
      }
      else {
        options.showAlert(data?.error || '保存失败', 'danger')
      }
      ...
      qqFrameworkSyncConfigSaving.value = false
    }
  }

  async function handleSyncAllQqFramework() {
    qqFrameworkSyncSyncing.value = true
    try {
      const { data } = await api.post('/api/admin/qq-framework-sync/sync-all', {}, { timeout: 120000 })
      if (data?.ok && data.data) {
        const summary = data.data
        const parts: string[] = []
        parts.push(`拉取 ${summary.total ?? 0} 条，有效 ${summary.valid ?? 0}`)
        parts.push(`新增 ${summary.added ?? 0}，更新 ${summary.updated ?? 0}，启动 ${summary.started ?? 0}`)
        if ((summary.planApplied ?? 0) > 0)
          parts.push(`默认方案应用 ${summary.planApplied} 个`)
        if (summary.errors?.length)
          parts.push(`失败 ${summary.errors.length}`)
        options.showAlert(`同步完成：${parts.join('，')}`, 'primary')
      }
      else {
        options.showAlert(data?.error || '同步失败', 'danger')
      }
    }
    catch (e: any) {
      options.showAlert(e?.response?.data?.error || `同步失败: ${e.message || '未知错误'}`, 'danger')
    }
    finally {
      qqFrameworkSyncSyncing.value = false
    }
  }
  ...
  return {
    ...
    qqFrameworkSyncConfigSaving,
    qqFrameworkSyncSyncing,
    loginLinksSaving,
    loginLogoUploading,
    ...
    loadQqFrameworkSyncConfig,
    handleSaveQqFrameworkSyncConfig,
    handleSyncAllQqFramework,
    loadLoginLinks,
