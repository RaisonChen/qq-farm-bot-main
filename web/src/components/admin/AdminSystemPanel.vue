...
import type { CaptureConfig, QqFrameworkSyncConfig, SystemConfig } from '@/composables/useAdminSystemConfig'
...
const props = withDefaults(defineProps<{
  section: 'system' | 'capture' | 'qq-framework-sync'
...
const localQqFrameworkSyncConfig = defineModel<QqFrameworkSyncConfig>('localQqFrameworkSyncConfig', { required: false })
</script>
...
<template>
  <div class="space-y-4">
    ...
    <div class="space-y-4">
      ...
      <div v-if="props.section === 'qq-framework-sync'" class="border border-gray-200 rounded-lg bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        <h4 class="mb-3 flex items-center gap-2 text-base text-gray-900 font-bold dark:text-gray-100">
        ...
        <div class="mb-3 rounded-lg bg-gray-50 px-4 py-3 text-xs text-gray-600 dark:bg-gray-900/40 dark:text-gray-300">
          从已部署的 QQ 框架 Code 服务同步账号：按 QQ 号匹配，存在则更新、不存在则添加，同步后自动运行；开启「掉线重连」后，由该模块同步得到的账号一旦离线会自动拉取新 code 并重连。
        </div>
        ...
        <div class="grid mb-3 gap-3 md:grid-cols-3">
          <div class="rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-700 dark:bg-gray-900/40 dark:text-gray-200">
            ...
            <div class="mt-1 font-semibold truncate">
              {{ localQqFrameworkSyncConfig?.serverUrl || '未配置' }}
            </div>
          </div>
          <div class="rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-700 dark:bg-gray-900/40 dark:text-gray-200">
            <div class="text-xs text-gray-500 dark:text-gray-400">
            ...
            <div class="mt-1 font-semibold">
              {{ localQqFrameworkSyncConfig?.apiToken || localQqFrameworkSyncConfig?.tokenConfigured ? '已配置' : '未配置' }}
            </div>
          </div>
          <div class="rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-700 dark:bg-gray-900/40 dark:text-gray-200">
            <div class="text-xs text-gray-500 dark:text-gray-400">
            ...
            <div class="mt-1 font-semibold">
              {{ localQqFrameworkSyncConfig?.autoReconnect ? '已开启' : '已关闭' }}
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 text-sm">
          <BaseInput
            v-model="localQqFrameworkSyncConfig!.serverUrl"
            label="服务地址"
            type="text"
          ...
          />
          <BaseInput
            v-model="localQqFrameworkSyncConfig!.apiToken"
            label="API Token"
            type="password"
            :placeholder="localQqFrameworkSyncConfig?.tokenConfigured ? '已配置，留空保持不变' : '请输入访问 QQ 框架服务的 Token'"
            class="col-span-2"
          />
          <div class="col-span-2">
            <BaseSwitch
              v-model="localQqFrameworkSyncConfig!.autoReconnect"
              label="掉线重连（仅针对通过本模块添加/更新的账号）"
            />
          </div>
        </div>
        ...
        <div class="mt-3 flex flex-wrap justify-end gap-2">
          <BaseButton
            variant="secondary"
            size="sm"
            :loading="qqFrameworkSyncSyncing"
            @click="$emit('syncAllQqFramework')"
          >
            <span class="i-carbon-sync" />
            一键同步
          </BaseButton>
          <BaseButton
            v-if="showSave"
            variant="primary"
            size="sm"
            :loading="qqFrameworkSyncConfigSaving"
            @click="$emit('saveQqFrameworkSync')"
          >
            保存
          </BaseButton>
        </div>
